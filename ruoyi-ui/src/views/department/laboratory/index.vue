    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['department:laboratory:edit']"
        >修改</el-button>
      </el-col>
      ...
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['department:laboratory:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="化验检查" prop="laboratoryExamination">
          <el-input v-model="form.laboratoryExamination" placeholder="请输入化验检查" />
        </el-form-item>
...
<script>
import { listLaboratory, getLaboratory, delLaboratory, addLaboratory, updateLaboratory, exportLaboratory } from "@/api/department/laboratory";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getLaboratory(studentId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除化验科编号为"' + studentIds + '"的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
        return delLaboratory(studentIds);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有化验科数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
