    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['department:dental:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['department:dental:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['department:dental:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['department:dental:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="dentalList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="龋齿" prop="dentalCaries">
          <el-input v-model="form.dentalCaries" placeholder="请输入龋齿" />
        </el-form-item>
        <el-form-item label="缺齿" prop="missingTeeth">
          <el-input v-model="form.missingTeeth" placeholder="请输入缺齿" />
        </el-form-item>
        <el-form-item label="齿槽" prop="alveolar">
          <el-input v-model="form.alveolar" placeholder="请输入齿槽" />
        </el-form-item>
...
<script>
import { listDental, getDental, delDental, addDental, updateDental, exportDental } from "@/api/department/dental";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listDental(this.queryParams).then(response => {
        this.dentalList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getDental(studentId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.studentId != null) {
            updateDental(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addDental(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除牙科编号为"' + studentIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delDental(studentIds);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有牙科数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.exportLoading = true;
          return exportDental(queryParams);
        }).then(response => {
